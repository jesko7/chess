...
pub fn get_sliding_moves(
    ...
    legal_squares
        .iter()
        .map(|square| (square.0 as u8, square.1 as u8))
        .collect::<Vec<(u8, u8)>>()
}

pub fn get_rook_moves(start_square: (u8, u8), color: u8, board: &Board) -> Vec<(u8, u8)> {
    let dir_offsets = vec![(-1, 0), (1, 0), (0, -1), (0, 1)];
...
pub fn get_bishop_moves(start_square: (u8, u8), color: u8, board: &Board) -> Vec<(u8, u8)> {
    let dir_offsets = vec![(-1, -1), (1, -1), (-1, 1), (1, 1)];
    get_sliding_moves(start_square, color, board, dir_offsets)
}
...
pub fn get_pawn_moves(start_square: (u8, u8), color: u8, board: &Board) -> Vec<(u8, u8)> {
    let mut legal_squares = vec![];
    ...
    let coord = (start_square.0 as i32, start_square.1 as i32 + dir);
    if coord.1 < 8 && coord.1 >= 0 {
        let coord = (coord.0 as u8, coord.1 as u8);
        if board.pieces[square_to_index(coord)].is_none() {
            legal_squares.push(coord);
        }
    }

    let coord = (start_square.0 as i32, start_square.1 as i32 + dir * 2);
    if coord.1 < 8 && coord.1 >= 0 {
        let coord = (coord.0 as u8, coord.1 as u8);
        if board.pieces[square_to_index(coord)].is_none()
            && !board.pieces[square_to_index(start_square)]
                .clone()
                .unwrap()
                .has_moved
        {
    ...
    let coord = ((start_square.0 - 1) as i32, start_square.1 as i32 + dir);
    if coord.1 < 8 && coord.1 >= 0 && coord.0 < 8 && coord.0 >= 0 {
        let coord = (coord.0 as u8, coord.1 as u8);
        println!("piece: {:?}", coord);
        if let Some(piece) = board.pieces[square_to_index(coord)].clone() {
            if piece.get_color() != color {
                legal_squares.push(coord);
            }
        }
...
impl Piece {
    ...
    pub fn get_legal_moves(&self, board: &Board) -> Vec<(u8, u8)> {
        match self.get_piece() {
            ...
            PAWN => get_pawn_moves(self.square, self.get_color(), board),
            other => vec![],
        }
    }
}
